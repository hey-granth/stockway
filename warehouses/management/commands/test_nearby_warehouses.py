...
class Command(BaseCommand):
    ...
    def check_postgis(self):
        ...
        with connection.cursor() as cursor:
            ...
            if result:
                self.stdout.write(self.style.SUCCESS(
                    f'✓ PostGIS is installed (version: {result[1]})'
                ))
            else:
        ...
        with connection.cursor() as cursor:
            ...
            if results:
                for result in results:
                    self.stdout.write(self.style.SUCCESS(
                        f'✓ Spatial index found: {result[0]}'
                    ))
                    self.stdout.write(f'  Index definition: {result[1][:80]}...')
            else:
                self.stdout.write(self.style.ERROR(
                    '✗ No spatial index found on location field!'
                ))
                self.stdout.write(self.style.WARNING(
    ...
    def test_nearby_query(self, lat, lon, radius):
        ...
        if count > 0:
            ...
            self.stdout.write(self.style.WARNING(
                f'  ⚠ No warehouses found within {radius} km'
            ))
            self.stdout.write(f'  Total warehouses in database: {Warehouse.objects.count()}')
            self.stdout.write(f'  Active & approved: {Warehouse.objects.filter(is_active=True, is_approved=True).count()}')
    ...
    def benchmark_query(self, lat, lon, radius, iterations=5):
        ...
        if avg_time < 50:
            ...
            self.stdout.write(self.style.WARNING(
                f'  ⚠ Performance could be improved (> 200ms)'
            ))
